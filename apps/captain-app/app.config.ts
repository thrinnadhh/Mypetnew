...
export default ({ config }: ConfigContext): ExpoConfig => ({
  ...config,
  name: 'MyPet Captain',
  slug: 'mypetnew-captain',
  scheme: 'mypet-captain',
  version: '0.1.0',
  ...
  android: {
    package: `in.mypetnew.captain${environment === 'production' ? '' : `.${environment}`}`,
    googleServicesFile: readEnv('GOOGLE_SERVICES_JSON')
  },
  ios: {
    bundleIdentifier: `in.mypetnew.captain${environment === 'production' ? '' : `.${environment}`}`,
