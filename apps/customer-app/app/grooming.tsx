...
type ServicePreview = { name: string; price: string; duration: string; image: string; badge?: string }
...
export default function GroomingScreen() {
  ...
  return (
    <Page showBack title="Grooming Services">
      ...
      <View style={styles.topRow}><Text style={text.muted}>12 services</Text><Text style={styles.sort}>↕ Sort   ☷ Filters</Text></View>
      <View style={styles.grid}>{services.map((service) => <View key={service.name} style={[styles.card, { width: cardWidth }]}><View style={styles.imageWrap}><Image source={{ uri: service.image }} style={styles.image} />{service.badge ? <View style={styles.badge}><Badge label={service.badge} tone={service.badge === 'Sale' ? 'red' : 'amber'} /></View> : null}</View><View style={styles.body}><Text style={styles.name}>{service.name}</Text><Text style={text.tiny}>◷ {service.duration}</Text><Text style={styles.price}>{service.price}</Text><PrimaryButton compact disabled label="Book later" /></View></View>)}</View>
      <InfoCard tone="blue" title="Frontend preview"><Text style={text.muted}>Appointment booking is intentionally disabled while Sprint 1 is active. The layout is production-sized now so the later API can bind without redesign.</Text></InfoCard>
...
const styles = StyleSheet.create({
  ...
  sort: { color: palette.primary, fontSize: 12, fontWeight: '800' },
