...
describe('M7 SQLite Bootstrap and Migrations', () => {
  it('performs clean bootstrap to schema version 5 and creates durable draft tables', async () => {
    ...
    expect(tableNames).toContain(TABLE_LOCAL_CATALOG_DRAFTS);
    expect(tableNames).toContain(TABLE_CATALOG_IDENTITY_MAPPINGS);
    expect(tableNames).toContain(TABLE_CATALOG_MEDIA_JOBS);

    await db.close();
  });

  it('runs migration chain forward through v5 and is idempotent at v5', async () => {
    const db = createNodeSqliteDatabase(':memory:');
    ...
    const res5 = await runMigrations(db, 5);
    expect(res5.currentVersion).toBe(5);
    expect(res5.appliedVersions).toEqual([5]);

    const res6 = await runMigrations(db, 5);
    expect(res6.currentVersion).toBe(5);
    expect(res6.appliedVersions).toEqual([]);
