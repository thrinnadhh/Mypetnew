...
class CustomerAppointmentApiTest {
    ...
    fun `public service discovery and customer appointment lifecycle use canonical ownership`() {
        ...
        val held = mockMvc.post("/api/v1/customer/appointments") {
            ...
            content = """{"outletId":"${outlet.id}","serviceId":"${service.id}","petId":"${petA.id}","slotId":"${slot.id}","slotStartsAt":"${slot.startsAt}","slotEndsAt":"${slot.endsAt}","pincode":"517501","paymentMethod":"PAY_AT_PROVIDER","notes":"Sensitive paws"}"""
        }.andExpect {
            status { isCreated() }
    ...
    @Test
    fun `appointment API rejects unauthenticated foreign pet stale PIN and stale slot timestamps`() {
        ...
        mockMvc.post("/api/v1/customer/appointments") {
            ...
            contentType = MediaType.APPLICATION_JSON
            content = """{"outletId":"${outlet.id}","serviceId":"${service.id}","petId":"${foreignPet.id}","slotId":"${slot.id}","slotStartsAt":"${slot.startsAt}","slotEndsAt":"${slot.endsAt}","pincode":"517501"}"""
        ...
        mockMvc.post("/api/v1/customer/appointments") {
            ...
            contentType = MediaType.APPLICATION_JSON
            content = """{"outletId":"${outlet.id}","serviceId":"${service.id}","petId":"${ownedPet.id}","slotId":"${slot.id}","slotStartsAt":"${slot.startsAt}","slotEndsAt":"${slot.endsAt}","pincode":"517502"}"""
        }.andExpect {
            status { isConflict() }
        ...
        mockMvc.post("/api/v1/customer/appointments") {
            header("Authorization", "Bearer ${customerA.accessToken}")
            header("Idempotency-Key", "stale-slot-time")
            contentType = MediaType.APPLICATION_JSON
            content = """{"outletId":"${outlet.id}","serviceId":"${service.id}","petId":"${ownedPet.id}","slotId":"${slot.id}","slotStartsAt":"${slot.startsAt.plusSeconds(60)}","slotEndsAt":"${slot.endsAt.plusSeconds(60)}","pincode":"517501"}"""
        }.andExpect {
            status { isConflict() }
            jsonPath("$.code") { value("APPOINTMENT_SLOT_STALE") }
        }
        ...
        mockMvc.post("/api/v1/customer/appointments") {
            ...
            contentType = MediaType.APPLICATION_JSON
            content = """{"outletId":"${outlet.id}","serviceId":"${service.id}","petId":"${ownedPet.id}","slotId":"${slot.id}","slotStartsAt":"${slot.startsAt}","slotEndsAt":"${slot.endsAt}","pincode":"012345"}"""
        }.andExpect {
            status { isBadRequest() }
